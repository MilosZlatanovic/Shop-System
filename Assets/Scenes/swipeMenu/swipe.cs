...
public class swipe : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (runIt)
        {
            Editbeautifully(distance, pos, takeTheBtn);
            time += Time.deltaTime;
        ...
        for (int i = 0; i < pos.Length; i++)
        {
            if (scroll_pos < pos[i] + (distance / 2) && scroll_pos > pos[i] - (distance / 2))
            {
                ...
                imageContent.transform.GetChild(i).GetComponent<Image>().color = colors[1];
                 imageContent.transform.GetChild(i).gameObject.AddComponent<Text>();
    ...
    private void Editbeautifully(float distance, float[] pos, Button btn)
    {
